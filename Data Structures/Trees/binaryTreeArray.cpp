...
template <typename T>
void preorderTraversal(vector<T> &tree, unsigned int index = 0)
{
    if (index >= tree.size())
        return;
    cout << tree[index] << " ";
...
template <typename T>
void inorderTraversal(vector<T> &tree, unsigned int index = 0)
{
    if (index >= tree.size())
        return;
...
template <typename T>
void postorderTraversal(vector<T> &tree, unsigned int index = 0)
{
    if (index >= tree.size())
        return;
